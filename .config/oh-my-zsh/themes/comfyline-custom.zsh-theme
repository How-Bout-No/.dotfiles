...
function calcbat(){
    ...
    if [[ $(uname) == "Linux" ]]; then
    	...
    	if [[ $(pmset -g batt) =~ '([0-9]+)%\s+(discharging|charging)' ]]; then
        	capacity="${match[1]}"
        	stats="${match[2]}"
    	else
        	echo "Battery information not found or in unexpected format." >&2
    	fi
...
segments=("hostname" "username" "dir" "gitrepo" "venv" "currbat" "currtime" "currdate")
segment_ranks=($HOST_RANK $USER_RANK $DIR_RANK $GIT_RANK $VENV_RANK $BAT_RANK $TIME_RANK $DATE_RANK)
